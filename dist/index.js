...
class ForgeAPIClient {
    ...
    async makeRequest(endpoint, method = "GET", body) {
        ...
        if (!response.ok) {
            throw new McpError(ErrorCode.InternalError, `Forge API request failed: ${response.status} ${response.statusText}`);
        }
            ...
            try {
                switch (name) {
                    ...
                    case "update_deployment_script": {
                        const { serverId, siteId, content } = args;
                    ...
                    case "toggle_quick_deploy": {
                        const { serverId, siteId, enable } = args;
                        if (enable) {
